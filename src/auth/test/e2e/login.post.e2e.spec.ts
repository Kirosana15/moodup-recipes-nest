...
describe('POST /login', () => {
  ...
  describe('POST /login', () => {
    const request = (app: NestApplication, status: HttpStatus, user: { username: string; password: string }) => {
    ...
    it(`should return ${HttpStatus.OK} and set of tokens for user`, async () => {
      const res = await request(app, HttpStatus.OK, mockCredentials);
    ...
    it(`should return ${HttpStatus.UNAUTHORIZED} when login does not exist in database`, async () => {
      ...
      const res = await request(app, HttpStatus.UNAUTHORIZED, { username: 'Bob', password: mockPassword });
      expect(res.body.message).toMatch('Unauthorized');
    });

    it(`should return ${HttpStatus.UNAUTHORIZED} when password does not match the username`, async () => {
      validateUserSpy.mockRejectedValueOnce(null);
      const res = await request(app, HttpStatus.UNAUTHORIZED, { username: mockUsername, password: 'P4$s' });
      expect(res.body.message).toMatch('Unauthorized');
    });
  });
});
